...
$all_query = DB::table('reservations')->where('reservations.caterer_id', $user_id)->whereRaw('reservation_date < DATE_ADD( NOW(), INTERVAL 2 HOUR)')->whereRaw("DATE(reservation_date) = '$start'")->get();

$pending_query = DB::table('reservations')->where('reservations.caterer_id', $user_id)->whereRaw('reservation_date < DATE_ADD( NOW(), INTERVAL 2 HOUR)')->whereRaw("DATE(reservation_date) = '$start'")->where('reservation_status', "=", 'pending')->get();

$confirm_query = DB::table('reservations')->where('reservations.caterer_id', $user_id)->whereRaw('reservation_date < DATE_ADD( NOW(), INTERVAL 2 HOUR)')->whereRaw("DATE(reservation_date) = '$start'")->where('reservation_status', 'Confirm')->get();

$noshow_query = DB::table('reservations')->where('reservations.caterer_id', $user_id)->whereRaw('reservation_date < DATE_ADD( NOW(), INTERVAL 2 HOUR)')->whereRaw("DATE(reservation_date) = '$start'")->where('reservation_status', 'No Show')->get();

$cancel_query = DB::table('reservations')->where('reservations.caterer_id', $user_id)->whereRaw('reservation_date < DATE_ADD( NOW(), INTERVAL 2 HOUR)')->whereRaw("DATE(reservation_date) = '$start'")->where('reservation_status', 'Cancel')->get();
?>
        ...
        <div class="right_wrap_inner">
            <div class="informetion informetion_new">
                ...
                <div class="informetion_top">
                    <div class="tatils"> <span class="personal">Today's Reservations</span></div>

                    <div class="tabel_option">
                        <ul class="tabel_option_li">
                            <li class="{{Request::is('reservation/todayorders/all') ? 'active' :''}}"><a href="{{HTTP_PATH.'reservation/todayorders/all'}}">ALL ({{$all_query ? count($all_query) :'0'}})</a></li>
                            <li class="{{Request::is('reservation/todayorders/new') ? 'active' :''}}"><a href="{{HTTP_PATH.'reservation/todayorders/new'}}">NEW ({{$pending_query ? count($pending_query):'0'}})</a></li>
                            <li class="{{Request::is('reservation/todayorders/confirm') ? 'active' :''}}"><a href="{{HTTP_PATH.'reservation/todayorders/confirm'}}">CONFIRMED ({{$confirm_query ? count($confirm_query) :'0'}})</a></li>
                            <li class="{{Request::is('reservation/todayorders/noshow') ? 'active' :''}}"><a href="{{HTTP_PATH.'reservation/todayorders/noshow'}}">No Show ({{$noshow_query ? count($noshow_query) :'0'}})</a></li>
                            <li class="{{Request::is('reservation/todayorders/cancel') ? 'active' :''}}"><a href="{{HTTP_PATH.'reservation/todayorders/cancel'}}">CANCELLED ({{$cancel_query ? count($cancel_query):'0'}})</a></li>
                        </ul>  
                    </div>
...
//                                                                print_r($offers);
                                                            ...
                                                            <div class="btnn"><a href="javascript:void(0)" onclick="updatetime('{{$data->id}}')">Save</a></div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
